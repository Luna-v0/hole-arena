...
export const GameConfigurator = () => {
  ...
  const handleCreateGame = async (bots: Bot[], humanPlayers: number) => {
    try {
      ...
      <div className="w-full max-w-2xl bg-slate-800 p-8 rounded-lg shadow-lg">
        ...
        {configMode === 'preset' ? (
          <div className="flex flex-col space-y-4">
            <button onClick={() => handlePreset(4, 0)} className="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold py-3 px-6 rounded-lg text-xl transition duration-300">4 Bots (Random)</button>
            <button onClick={() => handlePreset(1, 1)} className="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold py-3 px-6 rounded-lg text-xl transition duration-300">1v1 Bot (Random)</button>
            <button onClick={() => setConfigMode('custom')} className="w-full bg-slate-600 hover:bg-slate-700 text-white font-bold py-3 px-6 rounded-lg text-xl transition duration-300">Custom</button>
          </div>
        ) : (
          <div>
            <div className="mb-6">
            ...
            <button 
              onClick={() => handleCreateGame(bots, humanPlayers)} 
            ...
            <button onClick={() => setConfigMode('preset')} className="w-full bg-slate-600 hover:bg-slate-700 text-white font-bold py-2 px-4 rounded-lg mt-2">Back to Presets</button>
          </div>
        )}
      </div>
    </div>
